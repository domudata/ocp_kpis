...
def upload_file(path_in_repo: str, content_bytes: bytes, commit_message: str):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return False, f"Erreur vérification existence ({r.status_code}) : {r.text[:200]}"
    except requests.exceptions.RequestException as e:
        return False, f"Erreur réseau (vérification) : {e}"
    ...
    try:
        ...
        return False, f"Erreur réseau (envoi) : {e}"

    if r.status_code in (200, 201):
        return True, "OK"
    return False, f"Échec envoi ({r.status_code}) : {r.text[:300]}"
