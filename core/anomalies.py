...
def _backlogs_non_caracterises(dfp_all: pd.DataFrame):
    ...
    planification) selon la nouvelle logique convenue : périmètre ZCOR,
    correspondance EXACTE (aucune addition avant/après le code)."""
    zcor = dfp_all[dfp_all["Type d'ordre"] == "ZCOR"].copy()
    non_prep = zcor[~zcor["Statut utilisateur"].apply(lambda x: match_exact_token(x, CODES_PREP_EXACT))]
...
def build_ano_map(dfp: pd.DataFrame, avf: pd.DataFrame, now_ts,
    ...
    avf_tot = avf_filtre.groupby("Poste travail princ.")["Avis"].count()
    avf_aprv = avf_filtre[avf_filtre["Statut utilisateur"].isin(["APRV", "APRV AVAU"])].groupby("Poste travail princ.")["Avis"].count()
    ...
    # ── Backlog préparation/planification — NOUVELLE LOGIQUE (convenue) ──
