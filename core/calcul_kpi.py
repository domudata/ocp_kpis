...
def calc_kpis(df_i: pd.DataFrame, av_i: pd.DataFrame, now_ts, posts: list,
    ...
    # ── MODIFICATION AGE EXÉCUTION (Inconnu exclu du denominator) ──
    total_age_ex = ex["<1 mois"] + ex["1 mois < <3 mois"] + ex[">3 mois"]
    ex["OT exécution <1 mois"] = np.where(total_age_ex == 0, 100.0, (ex["<1 mois"] / total_age_ex) * 100)
    ex["OT exécution 1mois< <3mois"] = np.where(total_age_ex == 0, 0.0, (ex["1 mois < <3 mois"] / total_age_ex) * 100)
    ex["OT exécution >3 mois"] = np.where(total_age_ex == 0, 0.0, (ex[">3 mois"] / total_age_ex) * 100)
    ...
    # ── MODIFICATION AGE PRÉPARATION (Inconnu exclu du denominator) ──
    total_age_pr = pr["<1 mois"] + pr["1 mois < <3 mois"] + pr[">3 mois"]
    pr["OT préparation <1 mois"] = np.where(total_age_pr == 0, 100.0, (pr["<1 mois"] / total_age_pr) * 100)
    pr["OT préparation 1mois< <3mois"] = np.where(total_age_pr == 0, 0.0, (pr["1 mois < <3 mois"] / total_age_pr) * 100)
    pr["OT préparation >3 mois"] = np.where(total_age_pr == 0, 0.0, (pr[">3 mois"] / total_age_pr) * 100)
    ...
    # ── MODIFICATION AGE PLANIFICATION (Inconnu exclu du denominator) ──
    total_age_pl = pl["<1 mois"] + pl["1 mois < <3 mois"] + pl[">3 mois"]
    pl["OT planification <1 mois"] = np.where(total_age_pl == 0, 100.0, (pl["<1 mois"] / total_age_pl) * 100)
    pl["OT planification 1mois< <3mois"] = np.where(total_age_pl == 0, 0.0, (pl["1 mois < <3 mois"] / total_age_pl) * 100)
    pl["OT planification >3 mois"] = np.where(total_age_pl == 0, 0.0, (pl[">3 mois"] / total_age_pl) * 100)
    ...
    # ── Dictionnaire numérateur/dénominateur mis à jour pour Age ──
    res['nd'] = {
        "TAUX_REALISATION_CORRECTIF/PT": (an["OT_CLOTURES"], an["TOTAL_OT"]),
        "OT préparation <1 mois": (pr["<1 mois"], total_age_pr),
        "OT préparation 1mois< <3mois": (pr["1 mois < <3 mois"], total_age_pr),
        "OT préparation >3 mois": (pr[">3 mois"], total_age_pr),
        "OT planification <1 mois": (pl["<1 mois"], total_age_pl),
        "OT planification 1mois< <3mois": (pl["1 mois < <3 mois"], total_age_pl),
        "OT planification >3 mois": (pl[">3 mois"], total_age_pl),
        "OT exécution <1 mois": (ex["<1 mois"], total_age_ex),
        "OT exécution 1mois< <3mois": (ex["1 mois < <3 mois"], total_age_ex),
        "OT exécution >3 mois": (ex[">3 mois"], total_age_ex),
