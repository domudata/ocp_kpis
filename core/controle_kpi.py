...
from core.constants import (
    ...
    CODES_PREP_EXACT, CODES_PLAN_EXACT,
)
...
def eval_ages_kpi(df_all: pd.DataFrame, phase: str, now_ts) -> tuple:
    ...
    Retourne (df_inf, df_1_3, df_sup) :
      - df_inf : KPI < 1 mois (OUI si <= 30j, NON si > 30j)
      - df_1_3 : Part de la tranche 1-3 mois (OUI si 30 < j <= 90)
      - df_sup : Part de la tranche > 3 mois (OUI si j > 90)
    """
    if df_all.empty:
    ...
    if phase == "prep":
        ...
        _contient_atpl = statut_usr.str.contains("ATPL", case=False, na=False) | statut_usr.apply(lambda x: match_exact_token(x, CODES_PLAN_EXACT))
        scope = df_all[(type_ordre == "ZCOR") & (statut_sys == "LANC") & (sopl == 0) & _contient_atpl].copy()
        age_col = "amlp"
        cat_col = "alp"
    ...
    # ── KPI <1 mois (Cible >= 80% : Conforme si <= 30j, Anomalie si > 30j) ──
    df_inf = scope.copy()
    kpi_inf_name = f"{prefix} <1 mois"
    df_inf["KPI"] = kpi_inf_name
    df_inf["Nom_KPI_Complet"] = kpi_inf_name
    is_inf = cats.isin(["<1 mois", "Inconnu"]) | (ages <= 30)
    df_inf["Résultat"] = np.where(is_inf, "OUI", "NON")
    df_inf["Motif_NON"] = np.where(
        is_inf,
        "",
        ages.apply(lambda a: f"Âge {prefix.lower()} > 1 mois ({int(a)} j. / {a/30:.1f} mois)")
    )

    # ── KPI 1mois< <3mois (Cible <= 15% : Anomalie si 30 < j <= 90) ──
    df_1_3 = scope.copy()
    kpi_1_3_name = f"{prefix} 1mois< <3mois"
    df_1_3["KPI"] = kpi_1_3_name
    df_1_3["Nom_KPI_Complet"] = kpi_1_3_name
    is_in_1_3 = (cats == "1 mois < <3 mois") | ((ages > 30) & (ages <= 90))
    # Pour un indicateur LOWER_BETTER, les OT dans la tranche représentent les anomalies du backlog
    df_1_3["Résultat"] = np.where(is_in_1_3, "NON", "OUI")
    df_1_3["Motif_NON"] = np.where(
        is_in_1_3,
        ages.apply(lambda a: f"OT {prefix.lower()} commençant à vieillir (tranche 1 à 3 mois : {int(a)} j.)"),
        ""
    )

    # ── KPI >3 mois (Cible <= 5% : Anomalie si j > 90) ──
    df_sup = scope.copy()
    kpi_sup_name = f"{prefix} >3 mois"
    df_sup["KPI"] = kpi_sup_name
    df_sup["Nom_KPI_Complet"] = kpi_sup_name
    is_in_sup = (cats == ">3 mois") | (ages > 90)
    # Pour un indicateur LOWER_BETTER, les OT > 90j représentent les anomalies critiques du backlog
    df_sup["Résultat"] = np.where(is_in_sup, "NON", "OUI")
    df_sup["Motif_NON"] = np.where(
        is_in_sup,
        ages.apply(lambda a: f"OT bloqué en {prefix.lower().replace('ot ', '')} depuis plus de 3 mois ({int(a)} j. / {a/30:.1f} mois)"),
        ""
    )
