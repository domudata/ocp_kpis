...
def contient_mot(t, lm):
    """Vérifie si le texte t contient l'un des mots de la liste de listes lm."""
    t = str(t)
...
def excr(df):
    ...
    if "Poste travail princ." in df.columns:
        return df[
            ~df["Poste travail princ."].astype(str).str.contains("cresseur", case=False, na=False)
        ].copy()
    return df
...
def prepare_data(ot_bytes, av_bytes, date_str):
    ...
    # Postes SF1/SF2
    apm = sorted(
        df[df["Poste travail princ."].astype(str).str.startswith(("SF1", "SF2"), na=False)]
        ["Poste travail princ."].dropna().unique().tolist()
    )

    return df, avf, apm, now_ts, av_total_per_poste
