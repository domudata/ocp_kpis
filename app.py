...
# ── Imports protégés : affiche l'erreur RÉELLE dans l'app si un import échoue
try:
    ...
    # Import du module KPI (support de fallback si renommé en kpi.py ou calcul_kpi.py)
    try:
...
# ── CALCUL SCORE GLOBAL DIVISION (SF1 / SF2) ─────────────────────────────────
def calc_score_division(postes, liste_kpi, ano_map, ckdf):
    ...
    for kpi in liste_kpi:
        ...
        # KPI d'âge du backlog : conservation de la moyenne directe des postes
        ...
        # Règle OT Correctif : Non / (Oui + Non)
        if kpi == "OT Correctif":
...
def main() -> None:
    ...
    try:
        ...
        # Construction de ano_map avant l'utilisation dans le calcul des divisions
        ...
        # Calcul des scores SF1 et SF2 selon la nouvelle logique
        ...
        tot_p["Score Performance"] = "%.2f" % calc_score_division(vp, QK, ano_map, ckdf)
        prows.append(tot_p)
        ...
        tot_q["Score Qualite"] = "%.2f" % calc_score_division(vp, PK, ano_map, ckdf)
        qrows.append(tot_q)
        ...
        for poste in vp:
            ...
            for kpi in ALL_KPI:
                ...
                status = "non_vert" if nb_anom == 0 else ("oui_vert" if conforme else "oui_rouge")
        ...
        for poste in vp:
            ...
            if vals:
                ...
                stars = max(0, min(5, round(score_global / 20)))
            else:
                score_global = None
                stars = 0
            poste_stars[poste] = {"score": score_global, "stars": stars}

        avg_p_score = float(tot_p.get("Score Performance", 0))
        avg_q_score = float(tot_q.get("Score Qualite", 0))
        ...
        with tabs[4]:
            n_dates = hist_df["Date"].nunique() if not hist_df.empty and "Date" in hist_df.columns else 0
            with st.expander(f"📁 Historique : {n_dates} date(s) enregistrée(s) — cliquez pour détails", expanded=(n_dates < 2)):
                ...
                if n_dates < 2:
                    st.info(f"ℹ️ Il faut au moins 2 dates pour calculer des variations. Actuellement : {n_dates} date(s).")
                else:
                    st.success(f"✅ {n_dates} dates enregistrées sur GitHub.")
                try:
        ...
        with tabs[5]:
            ...
            try:
                ...
                st.error(f"❌ Module de publication des rapports indisponible : {_e_imp}")

            if _publication_ok and not _github_configured():
                st.caption("⚠️ Publication GitHub non configurée.")
            ...
            if _publication_ok and (_launch_publish or _launch_dry):
                ...
                with _status_area.container():
                    if _launch_dry:
                        st.success(f"✅ Génération test terminée : {_ok_pdf}/{len(_results)} PDF, {_ok_xlsx}/{len(_results)} Excel.")
                    else:
                        st.success(f"✅ {_ok_pub}/{len(_results)} postes publiés sur GitHub.")
                            ...
                            try:
                                ...
                                for _r in _results:
                                    if not _r.get("pdf_published"):
                                        continue
                                    _dossier = "".join(c if c.isalnum() or c in "-_" else "_" for c in str(_r["poste"]))
                                    _base = f"presentation/{_dossier}"
                                    _fichiers.append({
                                        "poste": _r["poste"],
                                        "dossier": _dossier,
                                        "pdf": f"{_base}/rapport.pdf",
                                        "pdf_url": f"https://raw.githubusercontent.com/{_repo}/{_branch}/{_base}/rapport.pdf" if _repo else "",
                                    })
                                _payload = {
                                ...
                                if _pa_resp.status_code in (200, 201, 202):
                                    st.success(f"☁️ Enregistrement OneDrive déclenché : {len(_fichiers)} rapport(s) envoyés.")
        ...
        st.error("Erreur lors du chargement des données : %s" % str(e))
        st.markdown('<div class="es">Veuillez vérifier que les fichiers ot.xlsx et avis.xlsx sont présents.</div>', unsafe_allow_html=True)

    st.markdown('<div class="footer">Bureau Méthodes Maroc Chimie 2026</div>', unsafe_allow_html=True)
