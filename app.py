...
def main() -> None:
    ...
    try:
        ...
        # CORRIGÉ (sur demande explicite) : règle UNIFORME pour TOUS les
        # KPI, y compris les 9 KPI d'âge (Préparation/Planification/
        # Exécution × 3 tranches) — l'ancienne exception _AGE_KPIS (qui
        # utilisait une moyenne simple des valeurs pour préserver la
        # somme à 100% des 3 tranches) est SUPPRIMÉE. Chaque KPI suit
        # maintenant exactement la même règle : gscore() par cellule
        # (rouge=0/conforme=1), puis nombre de cellules conformes / nombre
        # de cellules valides × 100.
        # ⚠️ Conséquence : les 3 tranches d'âge (<1 mois + 1-3 mois +
        # >3 mois) ne somment plus nécessairement à 100% dans cette ligne
        # Total general, contrairement à avant.
        ...
        # CORRIGÉ : calculé DIRECTEMENT sur TOUTES les cellules KPI
        # Performance de tous les postes sélectionnés (pas à partir des
        # valeurs déjà agrégées tot_p[k], pas une moyenne des scores par
        # poste). Réutilise calc_score_division(vp, QK), qui applique
        # exactement cette même règle cellule par cellule — garantit la
        # cohérence avec le Score par poste et par division.
