...
def main() -> None:
    ...
    try:
        ...
        # ── SCORES : METHODE UNIQUE ───────────────────────────────────────────── # REGLE : # Rouge / non conforme = 0 # Vert / orange / autre conforme = 1 # # Score = Somme des 0 et 1 / Nombre de KPI valides × 100 # # Cette methode est utilisee pour : # - Score Performance par poste # - Score Qualite par poste # - Score SF1 Performance / Qualite # - Score SF2 Performance / Qualite # - Total general Performance / Qualite def score_from_values(valeurs, liste_kpi): """ Calcule un score selon la methode : Rouge / non conforme = 0 Conforme = 1 Score = somme / nombre de KPI valides × 100 valeurs peut etre : - une Series correspondant a un poste - un dictionnaire de valeurs KPI """ total = 0 nombre_kpi = 0 for kpi in liste_kpi: if kpi not in valeurs.index: continue val = valeurs[kpi] # KPI vide = non pris en compte if pd.isna(val): continue # gscore retourne : # 0 = rouge / non conforme # 1 = conforme total += gscore(kpi, float(val), CIBLE[kpi]) nombre_kpi += 1 return round((total / nombre_kpi) * 100, 2) if nombre_kpi else 0 # ── SCORES PAR POSTE ───────────────────────────────────────────────────── pscores = {} qscores = {} for poste in ckdf.index: r = ckdf.loc[poste] # Score Performance du poste pscores[poste] = score_from_values(r, QK) # Score Qualite du poste qscores[poste] = score_from_values(r, PK) # ── POSTES SF1 / SF2 ───────────────────────────────────────────────────── sf1_posts = [ p for p in vp if str(p).startswith("SF1") ] sf2_posts = [ p for p in vp if str(p).startswith("SF2") ] # ── CALCUL SCORE DIVISION ──────────────────────────────────────────────── # Nouvelle methode : # # Pour CHAQUE cellule KPI de CHAQUE poste : # # Rouge = 0 # Conforme = 1 # # Score division = # Somme de tous les 0 et 1 # ---------------------- × 100 # Nombre de KPI valides def calc_score_division(postes, liste_kpi): total = 0 nombre_kpi = 0 for poste in postes: if poste not in ckdf.index: continue r = ckdf.loc[poste] for kpi in liste_kpi: if kpi not in r.index: continue val = r[kpi] # KPI vide = non pris en compte if pd.isna(val): continue # Rouge = 0 # Conforme = 1 total += gscore( kpi, float(val), CIBLE[kpi] ) nombre_kpi += 1 return ( round((total / nombre_kpi) * 100, 2) if nombre_kpi else 0 ) # ── SCORES SF1 ─────────────────────────────────────────────────────────── sf1_p = calc_score_division( sf1_posts, QK ) sf1_q = calc_score_division( sf1_posts, PK ) # ── SCORES SF2 ─────────────────────────────────────────────────────────── sf2_p = calc_score_division( sf2_posts, QK ) sf2_q = calc_score_division( sf2_posts, PK ) # ── ANOMALIES ──────────────────────────────────────────────────────────── ano_map = build_ano_map( dfp, avf, now_ts ) ano_p_rows = build_ano_rows( vp, ano_map, QK ) ano_q_rows = build_ano_rows( vp, ano_map, PK, fixed_zero=[ "OT Fiabilité", "Total Avis de Panne" ] ) ano_p_cols = [ "Poste de travail" ] + QK + [ "Total Anomalies" ] ano_q_cols = [ "Poste de travail" ] + PK + [ "Total Anomalies" ] anomaly_dfs = build_anomaly_dfs( dfp, avf, now_ts ) # ── EXPORT SIDEBAR : ANOMALIES ─────────────────────────────────────────── with st.sidebar: with st.expander( "📥 Export anomalies (OT + Avis)", expanded=False ): try: from core.export_anomalies import ( build_anomalies_workbook ) _xlsx_bytes = build_anomalies_workbook( anomaly_dfs, KPI_RESP_MAP, ACT_MAP ) st.download_button( "⬇️ Télécharger le fichier anomalies (.xlsx)", data=_xlsx_bytes, file_name=( f"anomalies_OT_Avis_" f"{fichier_date.replace('/', '-')}.xlsx" ), mime=( "application/vnd.openxmlformats-officedocument." "spreadsheetml.sheet" ), use_container_width=True, ) st.caption( "Contient 2 feuilles : " "Anomalies OT et Anomalies Avis, " "avec Responsable et Action recommandée, " "filtrées selon la période / poste / atelier / " "division sélectionnés." ) except Exception as _e: st.caption( f"Export indisponible : {_e}" ) # ── COLONNES TABLE PERFORMANCE / QUALITE ───────────────────────────────── pcols = [ "Poste de travail" ] + QK + [ "Score Performance" ] qcols = [ "Poste de travail" ] + PK + [ "Score Qualite" ] prows = [] qrows = [] # ── LIGNES PAR POSTE ───────────────────────────────────────────────────── for poste in ckdf.index: r = ckdf.loc[poste] # ── PERFORMANCE ────────────────────────────────────────────────────── prw = { "Poste de travail": poste } for k in QK: if k in r.index and pd.notna(r[k]): prw[k] = "%.1f" % float(r[k]) else: prw[k] = "nan" prw["Score Performance"] = "%.2f" % ( pscores.get(poste, 0) ) prows.append(prw) # ── QUALITE ────────────────────────────────────────────────────────── qrw = { "Poste de travail": poste } for k in PK: if k in r.index and pd.notna(r[k]): qrw[k] = "%.1f" % float(r[k]) else: qrw[k] = "nan" qrw["Score Qualite"] = "%.2f" % ( qscores.get(poste, 0) ) qrows.append(qrw) # ── LIGNE CIBLE PERFORMANCE ────────────────────────────────────────────── cible_p = { "Poste de travail": "CIBLE", "_t": "cible" } for k in QK: cible_p[k] = "%.0f" % CIBLE.get( k, 100 ) cible_p["Score Performance"] = "100" prows.append(cible_p) # ── LIGNE CIBLE QUALITE ────────────────────────────────────────────────── cible_q = { "Poste de travail": "CIBLE", "_t": "cible" } for k in PK: cible_q[k] = "%.0f" % CIBLE.get( k, 100 ) cible_q["Score Qualite"] = "100" qrows.append(cible_q) # ── TOTAL GENERAL PERFORMANCE ──────────────────────────────────────────── # # Chaque cellule KPI est evaluee : # # Rouge = 0 # Conforme = 1 # # Pour chaque KPI : # # Total KPI = # Nombre de postes conformes # ------------------------- × 100 # Nombre de postes valides # # Le Score Performance final = # # Somme de tous les resultats 0/1 # ------------------------------- × 100 # Nombre total de cellules KPI tot_p = { "Poste de travail": "Total general", "_t": "total" } total_score_p = 0 nombre_total_kpi_p = 0 for k in QK: conformes = 0 total_valides = 0 for poste in ckdf.index: r = ckdf.loc[poste] if k not in r.index: continue val = r[k] if pd.isna(val): continue resultat = gscore( k, float(val), CIBLE[k] ) # Pour le total affiche du KPI conformes += resultat total_valides += 1 # Pour le Score Performance global total_score_p += resultat nombre_total_kpi_p += 1 if total_valides > 0: tot_p[k] = "%.1f" % ( (conformes / total_valides) * 100 ) else: tot_p[k] = "nan" # Score Performance Total general tot_p["Score Performance"] = "%.2f" % ( (total_score_p / nombre_total_kpi_p) * 100 if nombre_total_kpi_p else 0 ) prows.append( tot_p ) # ── TOTAL GENERAL QUALITE ──────────────────────────────────────────────── # # Exactement la meme methode que Performance : # # Rouge = 0 # Conforme = 1 tot_q = { "Poste de travail": "Total general", "_t": "total" } total_score_q = 0 nombre_total_kpi_q = 0 for k in PK: conformes = 0 total_valides = 0 for poste in ckdf.index: r = ckdf.loc[poste] if k not in r.index: continue val = r[k] if pd.isna(val): continue resultat = gscore( k, float(val), CIBLE[k] ) # Total KPI affiche conformes += resultat total_valides += 1 # Score Qualite global total_score_q += resultat nombre_total_kpi_q += 1 if total_valides > 0: tot_q[k] = "%.1f" % ( (conformes / total_valides) * 100 ) else: tot_q[k] = "nan" # Score Qualite Total general tot_q["Score Qualite"] = "%.2f" % ( (total_score_q / nombre_total_kpi_q) * 100 if nombre_total_kpi_q else 0 )
