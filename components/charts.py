...
def show_pie_pair(piv_df, title_prefix):
    """Double pie chart : par statut OT + réalisés vs non réalisés."""
    ...
    fig.add_trace(go.Pie(
        labels=pie2_data.index, values=pie2_data.values, hole=0.5,
        textinfo='percent+label',
        texttemplate='%{label}<br>%{percent:.1%}<br>(%{value})',
        textposition='inside', insidetextorientation='radial',
        textfont=dict(size=14, color='white', family='Inter, sans-serif'),
...
def show_simple_pie(piv_df, title, keep_non_carac=False):
    """Pie chart simple avec légende horizontale."""
    df_copy = piv_df.copy()
    if not keep_non_carac and "NON CARACTERISE" in df_copy.columns:
        df_copy = df_copy.drop(columns=["NON CARACTERISE"])
    ...
    fig = go.Figure(go.Pie(
        labels=counts.index, values=counts.values, hole=0.4, sort=False,
...
def show_bar_comparison(kpi_name, values_series, cible, title=None):
    """Barres horizontales comparant les postes pour un KPI donné."""
    ...
    colors = []
    for v in values_series.values:
        if is_lower:
            colors.append("#10b981" if v <= cible else ("#f59e0b" if v <= cible * 1.5 else "#ef4444"))
        else:
            colors.append("#10b981" if v >= cible else ("#f59e0b" if v >= cible * 0.8 else "#ef4444"))

    fig = go.Figure(go.Bar(
        y=values_series.index,
        x=values_series.values,
        orientation='h',
        marker_color=colors,
        text=[f"{v:.1f}%" for v in values_series.values],
    ...
    fig.update_layout(
        ...
        height=max(300, len(values_series) * 35),
        margin=dict(l=200, r=60, t=50, b=30),
        yaxis=dict(tickfont=dict(size=11)),
        xaxis=dict(title="%", range=[0, max(110, values_series.max() * 1.1)]),
        showlegend=False
    )

    # Ligne cible
    fig.add_vline(x=cible, line_dash="dash", line_color="#1e3a5f", line_width=2,
